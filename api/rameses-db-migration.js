...
const buildModule = async (module) => {
  ...
  const unprocessedFiles = await migrationEm
...
const initialize = async (servicePath) => {
  const dbmConf = {
    type: process.env.dbm_type || "mysql",
    host: process.env.dbm_server || "localhost",
    user: process.env.dbm_user || "root",
    password: process.env.dbm_pass || "1234",
    database: process.env.dbm_name || "dbm",
  }
